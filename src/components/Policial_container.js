import styles from "./Policial_container.module.css";
...
const Policial_container = () => {
  const { documents: news } = useFetchDocuments("news");

  if (news) {
    var policial = news.filter((item) => item.theme === "policial");
  }
  return (
    <div className={styles.main_container}>
      <div className={styles.news_container}>
        ...
        <div className={styles.cards}>
          {policial &&
            policial.map((item) => <CardNews key={item.id} data={item} />)}
        </div>
      </div>
...
export default Policial_container;
